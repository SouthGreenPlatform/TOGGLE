...
for (my $i=0; $i<=$#listOfFiles; $i++)                                                                      # for each folder, create directories for different step of analysis
{
    ...
    elsif ($listOfFiles[$i]=~m/.+\/.+:$/)
    {
        ...
        {
            toolbox::exportLog("INFOS: $0 : Run singleAnalysisBwaMem.pl on $firstDir\n",1);
            my $singleCom = 'qsub -N singleAnalysis '.$options.' "singleAnalysisBwaMem.pl '.$firstDir.' '.$fileConf.' '.$refFastaFile.'"';
            ##DEBUG
        ...
        {
            toolbox::exportLog("INFOS: $0 : Run pairAnalysisBwaMem.pl on $firstDir\n",1);
            my $pairCom = 'qsub -N pairAnalysis '.$options.' "pairAnalysisBwaMem.pl '.$firstDir.' '.$fileConf.' '.$refFastaFile.'"';
            ##DEBUG
