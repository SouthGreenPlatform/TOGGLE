...
#    if ($listOfFiles[$i]=~m/^(.+):$/)
#    {
#        $listeOfPath=$1;                                                                                       # recovery of the current GATK's folder path
#    }
#    elsif ($listOfFiles[$i]=~m/(4_MAPPING)$/)
#    {
#        $MappingPath.="/4_MAPPING";
#        ##DEBUG toolbox::exportLog("DEBUG: $0 : if File directory eq 4_MAPPING : $listOfFiles[$i]\n",1);
#        my $copyCom = "cp $MappingPath/*.bam $bamDirPath.";                                 # command to move bam files just created into the directory appropriate for the pipeline
