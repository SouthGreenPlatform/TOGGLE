...
# bwa::bwaMem
##########################################
print LOG "INFOS: $0 : Start BWA mem\n";
$newDir = toolbox::changeDirectoryArbo($initialDir,4);                                                      # change for the bwa direcotry
##DEBUG print LOG "CHANGE DIRECTORY TO $newDir\n";
my $fileWithoutExtention = pairing::extractName($repairingList[0]);                                             # extract name of file without the extention
my $samFileOut = "$newDir"."/"."$fileWithoutExtention".".BWAMEM.sam";                                     # name for the output file of bwa sampe
$softParameters = toolbox::extractHashSoft($optionref,"BWA mem");                                      # recovery of specific parameters of bwa sampe
bwa::bwaMem($samFileOut,$refFastaFile,$repairingList[0],$repairingList[1],$fileWithoutExtention,$softParameters); # generate alignement in SAM format
