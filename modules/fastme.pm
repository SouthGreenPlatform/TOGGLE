...
################################################################################################
# sub fastme::fastme => to run fastme
################################################################################################
# arguments :
...
################################################################################################
# return boolean :
#	- 1 if fastme has runned correctly
#	- else 0
################################################################################################
sub fastme
{
	...
	{ ##Check if entry file exist and is not empty
          ...
          my $command=$fastme." $options -o $fileOut.nwk -i $fileIn -O $fileOut.mat ";
          ...
          {
               toolbox::exportLog("ERROR: fastme::fastme : Uncorrectly done\n",0);
               return 0;#Command not Ok
          }
     }
     else
     {
        toolbox::exportLog("ERROR: fastme::fastme : The file $fileIn is uncorrect\n",0);
...
=head1 SYNOPSIS
	...
	fastme::fastme($input,$output);                                                           

=head1 DESCRIPTION

This module is a set of functions related to fastme software,  L<https://www.cog-genomics.org/readseq2>
...
Return 1 if fastme has ran correctly else 0.

Example : 
C<fastme::fastme($vcfFile,$pedOut);> 	
